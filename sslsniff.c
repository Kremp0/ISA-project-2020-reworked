 ...
 * 
 * contanct1: xkremp01@stud.fit.vutbr.cz
 * contanct2: samuel.krempasky@gmail.com
 * 
 * xkremp01
 * used for SSL headers https://tls13.ulfheim.net/
 * some code was inspired by official examples from prof. Petr Matousek, 2020. Especially included libraries and main loop in main
 * 
 */
#include <stdio.h>
#include <stdlib.h>
...
//just a handle for printing out
bool isFile=true;
...
void computeDuration(connection *ptr){
    ...
    //some connections can theoretically have moths and years
...
void clearArray(char c[],int lenght){
...
/*
connection comparisonHeadIPV4(struct ip *my_ip){
    connection *ptr=globalConnections;

}

connection comparisonHeadIPV6(){
...
connection *checkConnections(const struct tcphdr *tcpPacket, struct ip *my_ip, const struct ip6_hdr *ipv6_header){//here is a problem
    ...
    while(ptr!=NULL){//iterates the linked-list 
        ...
        if (my_ip!=NULL){
            ...
            if (my_ip->ip_src.s_addr == ptr->clientIP.s_addr || my_ip->ip_dst.s_addr == ptr->clientIP.s_addr ){
                //ip comparison
                if(my_ip->ip_src.s_addr == ptr->serverIP.s_addr || my_ip->ip_dst.s_addr == ptr->serverIP.s_addr){
                    if (ntohs(tcpPacket->th_dport)==ptr->clientPort || ntohs(tcpPacket->th_sport)==ptr->clientPort ){
                        //port comparison, beacause there can be different ports and connections
...
void appendAndCreateConnection(const struct tcphdr *tcpPacket, struct ip *my_ip, struct tm *timestamp, unsigned long time_in_micros, const struct ip6_hdr *ipv6_header){//adds connection and initialize the state
...
void analyzeEstablishedConnectionHeadears(int data[],int size, char *serverName, u_int packetByteLenght, connection *ptr){
    ...
    */
    ...
    for(int i=size;i<packetByteLenght;i++){
        ...
        if (data[i]>=0x14 && data[i]<=0x17
            ...
            i=i+jumpLenght;//to "byte"-jump over data
...
void FSmach(const struct tcphdr *tcpPacket , struct ip *my_ip, u_int lenght,  int data[],int size, struct tm *timestamp, unsigned long time_in_micros, const struct ip6_hdr *ipv6_header){
    ...
    if (globalConnections==NULL)
        ...
        if(ptrMain!=NULL){//it has found a connection //it should also return a pointer to a connection
            ...
            //creates new connection to a linked list
            //if it is a new connection it should have a SYN packet

            if (tcpPacket->th_flags & TH_SYN){
                appendAndCreateConnection(tcpPacket, my_ip, timestamp, time_in_micros, ipv6_header);
            }
...
int main(int argc, char *argv[]){
    ...
    //this cycle was used by garant/proffessor
